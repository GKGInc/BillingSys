...
public class EdiService
{
    ...
    public EdiService(string connectionString, ILogger<EdiService> logger)
    {
    ...
    public async Task<ServiceResult<List<EdiCustomerBillingPreview>>> GetEdiMonthlyBillingPreviewAsync(
    ...
    private static List<EdiTradingPartnerSummary> ApplyTradingPartnerMappings(List<EdiTradingPartnerSummary> results)
    {
        ...
        foreach (var result in results)
        {
            ...
            if (result.TradingPartnerCode != "_")
            {
                result.BillType = "X12";
            }
            else
